...
while IFS=':' read -r group users; do
	...
	if [ "$1" == "-d" ]; then
		if grep -q "^$group:" /etc/group; then #check groups in a system (-q - quet)
			...
			for user in $(echo $users | tr ',' ' '); do #for each user delete his from the system
				sudo userdel $user
			done
			echo "Group $group deleted"
		fi
	else
		if ! grep -q "^$group:" /etc/group; then	#for each group add it
			sudo groupadd $group
		fi
		for user in $(echo $users | tr ',' ' '); do
			if ! id -u $user > /dev/null 2>&1; then	#if user doesn't exist
				sudo useradd -N $user
